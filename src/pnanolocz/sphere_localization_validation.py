"""Sphere-only MATLAB/Python localization validation."""
...
def _direct_sphere_stack(movie: np.ndarray, pixperfeat: float = 1.0) -> np.ndarray:
    ...
    result = localize(stack, np.vstack(rows), "sphere", pixperfeat)
    result = result[np.all(np.isfinite(result[:, :2]), axis=1)]
    if len(result):
...
def export_python_sphere_tables(
    input_dir: str | Path, output_dir: str | Path, *, pixperfeat: float = 1.0
) -> list[dict[str, Any]]:
    """Export sphere localization tables and a per-input manifest."""
    ...
    for path in _input_tiffs(input_path):
        ...
        try:
            ...
            if target.exists():
                ...
                continue
            table = _direct_sphere_stack(tifffile.imread(path), pixperfeat)
    ...
    (output_path / "manifest.json").write_text(
        json.dumps(
            {"method": "sphere", "pixperfeat": pixperfeat, "files": manifest}, indent=2
        ),
        encoding="utf-8",
    )
    return manifest
...
def _add_radius_columns(
    ...
    """Attach sphere radii using the exact matched source rows."""
    result = matched.copy()
    mi = result["matlab_index"].to_numpy(int)
    pi = result["python_index"].to_numpy(int)
    result["matlab_radius"] = mat.iloc[mi]["extra10"].to_numpy(float)
    result["python_radius"] = py.iloc[pi]["extra10"].to_numpy(float)
    result["delta_radius"] = result["python_radius"] - result["matlab_radius"]
    return result


def _radius_metrics(matched: pd.DataFrame) -> dict[str, float]:
    values = matched["delta_radius"].to_numpy(float)
    return {
        "radius_bias": float(np.mean(values)) if len(values) else np.nan,
        "radius_mae": float(np.mean(np.abs(values))) if len(values) else np.nan,
        "radius_rmse": float(np.sqrt(np.mean(values**2))) if len(values) else np.nan,
    }
...
def _plots(
    ...
    fig.savefig(output / f"{stem}_overlay.png", dpi=150)
    plt.close(fig)
    fig, ax = plt.subplots()
    if len(matched):
    ...
    fig.savefig(output / f"{stem}_residuals.png", dpi=150)
    plt.close(fig)
...
def compare_sphere_directories(
    matlab_dir: str | Path,
    python_dir: str | Path,
    ...
    *,
    ...
    """Compare sphere tables and write a timestamped report directory."""
    matlab_path, python_path, input_path = (
        Path(matlab_dir),
        Path(python_dir),
        Path(input_dir),
    )
    pairs = _validated_manifest_tables(matlab_path, python_path, "sphere")
    ...
    for source_name, table_name in pairs:
        ...
        matched = _add_radius_columns(matched, mat, py)
        stem = mat_file.stem
        ...
        summary.update(_radius_metrics(matched), source=stem)
    ...
    overall.update(_radius_metrics(combined))
    thresholds = {
        "match_rate_matlab_min": 0.90,
        "x_rmse_max": 0.25,
        "y_rmse_max": 0.25,
        "radius_rmse_max": 11.0,
    }
    ...
    def passes(metrics: dict[str, Any]) -> bool:
        return bool(
            ...
            and metrics["radius_rmse"] <= thresholds["radius_rmse_max"]
        )
    ...
    (run_dir / "comparison_report.md").write_text(
        "# Sphere MATLAB/Python Localization Comparison\n\n"
        + files.to_csv(index=False),
        encoding="utf-8",
    )
    if enforce_thresholds and not passed:
        raise RuntimeError(f"Sphere parity thresholds failed; report: {run_dir}")
    return run_dir


__all__ = ["compare_sphere_directories", "export_python_sphere_tables"]


def main() -> None:
    """Run the sphere exporter or directory comparison from the command line."""
    ...
    if args.command == "export":
        entries = export_python_sphere_tables(args.input, args.output)
        if any(entry["status"] != "ok" for entry in entries):
            raise SystemExit(1)
    else:
        compare_sphere_directories(
